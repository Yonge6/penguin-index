...
2026-09-16: Promote Products, Skills and DSH Ecosystem into three separate first-level navigation entries. Standalone ranking pages have channel-specific headings and no channel tab strip. Preserve existing page=opensource&channel=... deep links and period/filter functionality. Homepage may retain its compact channel preview tabs. The six-link navigation moves to a second row at 1200px and scrolls horizontally on narrow phones; active links must remain visible.

Add the exact attribution Powered by Hy4 at the very bottom of every page, below the footer methodology, as a quiet centered technology credit.

Pricing scope tabs stick directly below the fixed site header. Paginate both price datasets at 20 rows per page; controls appear above and below the table. Scope/search/filter/sort/currency changes reset to page one, page changes return to the results heading, and CSV exports all filtered rows.

Show methodology with the corresponding ranking rather than a shared mixed footer. Standalone ranking pages show expanded relevant notes; homepage preview cards have compact expandable notes. Global footer contains only Powered by Hy4. Use reader-friendly 数据更新 / Data updated consistently, including products/skills/DSH header summaries. Summary pattern: source, coverage, actual dataset update date. Do not claim daily 03:00 syncing until the intended upstream/site scheduling is confirmed and verified. Usage explains platform-only coverage and reference-only charts; prices distinguish OpenRouter USD rates from the Chinese-provider source dataset, disclose the dataset FX rate, tiered-price stars and missing values.

2026-09-17: Rename the website to AI 趋势榜 / AI Trend Rankings. Put the site name before the intact Tencent Technology + Penguin Intelligence logo pair in the shared header. Remove the entire homepage wave heading module; ranking cards start below the fixed header. Retain other ranking page headings. Header uses a second navigation row at 1400px; at 760px and below, stack the publisher pair beneath the site name with a 144px total fixed header. Update document metadata and chart attribution to the site name while preserving supplied publisher logos.

2026-09-17: Usage header contains only eyebrow, title and introductory sentence. Remove its source-summary and entire hero-data block (platform total, growth and metadata); preserve charts, exports and the relevant methodology below the ranking.

2026-09-17 latest navigation: 首页, 模型能力榜, 模型调用榜, 模型价格榜, 开源产品榜, Agent Skills榜. Restore benchmarks navigation and its coming-soon page; remove DSH from primary navigation, retaining its existing data and deep links. All ranking headers share PageHeading with a numbered English eyebrow (01 benchmarks, 02 usage, 03 pricing, 04 open source, 05 skills), exact requested heading, and one-sentence description. Homepage remains without a banner.

2026-09-17 latest footer specification: Each ranking has separate Data sources, Methodology and Disclaimer sections with user-supplied copy. Product and Agent Skills have distinct eligibility rules and disclosures. Source/coverage/update metadata lives below these notes; prices show both dataset dates separately. Benchmarks have no data notes while pending. Remove DSH completely from all visible selectors and routes; legacy channel=dsh resolves to product. Retain underlying archived data files rather than delete unrelated datasets.
...
2026-09-18: Keep the original pale-gray background for unselected day/week/month period filters on homepage, Open Source and Agent Skills rankings. Toolbar and pagination selects use a custom chevron inset slightly farther from the right edge. On the usage page, remove all decorative motto copy from the right snapshot card, keep both desktop one-week cards tight around their content, and show the selected dark-theme control with a dark background and white text.

2026-09-18: Standalone Pricing, Open Source and Agent Skills tables use a clearly distinct blue-gray header row against their white data rows; the contrast must remain visible on the live page.
