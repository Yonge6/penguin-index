...
export default function ModelLogo({item,lang='en',large=false}) {
  ...
  return <span className={'model-logo'+(large?' model-logo-large':'')} title={label}>
    {brand && failedFile!==brand.file ? <img src={import.meta.env.BASE_URL+'assets/models/'+brand.file} alt={label} width="24" height="24" loading="lazy" onError={()=>setFailedFile(brand.file)}/> : <FingerprintIcon size={24} role="img" aria-label={label}/>}
  </span>;
}
