...
export function readState() {
 ...
 return {priceScope:p.get('scope')==='china'?'china':'global',page:routes.includes(route)?route:'home',lang:['zh','en'].includes(p.get('lang'))?p.get('lang'):saved|| (navigator.language.startsWith('zh')?'zh':'en'),channel:['product','skill'].includes(p.get('channel'))?p.get('channel'):'product',period:['daily','weekly','monthly'].includes(p.get('period'))?p.get('period'):'daily'};
}
